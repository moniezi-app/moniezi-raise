...
export const normalizeCompanyEquityState = (raw: unknown, businessName = ''): CompanyEquityState => {
  ...
  return {
    ...
    portalConfig: {
      ...base.portalConfig,
      ...(obj.portalConfig || {}),
      enabled: !!obj.portalConfig?.enabled,
      portalBaseUrl: obj.portalConfig?.portalBaseUrl || '',
      apiBaseUrl: obj.portalConfig?.apiBaseUrl || '',
      companySlug: obj.portalConfig?.companySlug || '',
      issuerName: obj.portalConfig?.issuerName || '',
      adminEmail: obj.portalConfig?.adminEmail || '',
      lastSyncAt: obj.portalConfig?.lastSyncAt || '',
      notes: obj.portalConfig?.notes || '',
    },
    portalSubmissions: Array.isArray(obj.portalSubmissions) ? obj.portalSubmissions.map((submission, index) => ({
      id: String(submission.id || `eq_portal_submission_${index + 1}`),
      token: String(submission.token || ''),
      packageId: submission.packageId || '',
      submittedAt: submission.submittedAt || new Date().toISOString(),
      investorName: String(submission.investorName || 'Unnamed investor'),
      email: submission.email || '',
      phone: submission.phone || '',
      entityName: submission.entityName || '',
      desiredAmount: safeNumber(submission.desiredAmount, 0),
      instrumentType: submission.instrumentType || 'undecided',
...
export const createDemoEquityState = (businessName = 'MONIEZI Demo Studio'): CompanyEquityState => {
  ...
  return {
    ...
    portalConfig: {
      enabled: true,
      portalBaseUrl: 'https://invest.emyrgengroup.com',
      apiBaseUrl: 'https://investor-portal.emyrgengroup.workers.dev',
      companySlug: 'emyrgen-demo',
      issuerName: 'Eli Green',
      adminEmail: 'issuer@example.com',
      lastSyncAt: '',
      notes: 'Demo Cloudflare Investor Portal configuration. Replace with your real Cloudflare Pages/Worker URLs.',
    },
    portalSubmissions: [
      {
        id: 'eq_portal_submission_demo_1',
        token: 'demo-sent-package-token',
        packageId: 'eq_reservation_demo_package_sent',
        submittedAt: new Date().toISOString(),
        investorName: 'Victor Family Investor',
        email: 'victor.family@example.com',
        phone: '+1 555 010 1122',
        entityName: '',
        desiredAmount: 15000,
        instrumentType: 'common_stock',
        signatureName: 'Victor Family Investor',
        ipHash: 'demo_ip_hash_only',
        userAgent: 'Demo browser audit string',
        consentElectronicRecords: true,
        acknowledgmentIndicationOnly: true,
        acknowledgmentRisk: true,
        status: 'new',
        notes: 'Demo portal submission imported from the future Cloudflare endpoint.',
      },
    ],
  };
};
