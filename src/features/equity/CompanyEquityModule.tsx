...
import type {
  ...
  EquityPortalConfig,
  EquityPortalSubmission,
  EquityPortalSubmissionStatus,
  EquityIssuance,
...
type EquitySection = 'guide' | 'settings' | 'stakeholders' | 'issuance' | 'captable' | 'packages' | 'portal' | 'reservations' | 'safes';
...
const defaultPortalConfig = (): EquityPortalConfig => ({
  enabled: false,
  portalBaseUrl: '',
  apiBaseUrl: '',
  companySlug: '',
...
const buildPackageLink = (token?: string, portalBaseUrl?: string) => {
  const cleanToken = token || 'preview-token';
  const base = normalizeUrl(portalBaseUrl);
  if (base) return `${base}/p/${cleanToken}`;
  if (typeof window === 'undefined') return `https://your-domain.com/p/${cleanToken}`;
...
export function CompanyEquityModule({ equity, onChange, currencySymbol, defaultBusinessName, showToast }: Props) {
  ...
  const portalConfig = state.portalConfig || defaultPortalConfig();
  const portalSubmissions = state.portalSubmissions || [];
  const [portalImportText, setPortalImportText] = useState('');
  ...
  const updatePortalConfig = (patch: Partial<EquityPortalConfig>) => {
    onChange(prev => {
      const normalized = normalizeCompanyEquityState(prev, defaultBusinessName);
      return {
        ...normalized,
        portalConfig: {
          ...(normalized.portalConfig || defaultPortalConfig()),
          ...patch,
        },
      };
    });
  ...
  const portalPayloadFor = (record: Partial<EquityInvestmentReservation>) => {
    const packageRecord = buildPackageRecord(record);
    return {
      schemaVersion: 'moniezi-investor-portal-v1',
      company: {
        legalName: companyName,
        stateOfIncorporation: state.profile.stateOfIncorporation || '',
        slug: portalConfig.companySlug || '',
        issuerName: portalConfig.issuerName || '',
        adminEmail: portalConfig.adminEmail || '',
      },
      package: {
        token: packageRecord.packageToken,
        title: packageRecord.packageTitle,
        preparedFor: packageRecord.packagePreparedFor,
        investorName: packageRecord.investorName,
        email: packageRecord.email,
        phone: packageRecord.phone,
        entityName: packageRecord.entityName,
        desiredAmount: packageRecord.desiredAmount,
        instrumentType: packageRecord.instrumentType,
        minimumInvestment: packageRecord.packageMinimumInvestment || 0,
        pricePerShare: packageRecord.packagePricePerShare || 0,
        estimatedShares: packageRecord.packageEstimatedShares || 0,
        expirationDate: packageRecord.packageExpirationDate || '',
        privateMessage: packageRecord.packagePrivateMessage || '',
        offeringSummary: packageRecord.packageOfferingSummary || '',
        majorTerms: packageRecord.packageMajorTerms || '',
        riskText: packageRecord.packageRiskText || '',
      },
    };
  };

  const downloadPortalPackageJson = (record: Partial<EquityInvestmentReservation>) => {
    const payload = portalPayloadFor(record);
    const safeName = String(record.investorName || 'investor').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'investor';
    downloadText(`moniezi_${safeName}_portal_package_${todayIso()}.json`, JSON.stringify(payload, null, 2), 'application/json;charset=utf-8');
  };

  const copyPortalPackageJson = async (record: Partial<EquityInvestmentReservation>) => {
    const payload = JSON.stringify(portalPayloadFor(record), null, 2);
    try {
      await navigator.clipboard.writeText(payload);
      showToast('Portal package JSON copied.', 'success');
    } catch {
      downloadText(`moniezi_portal_package_${todayIso()}.json`, payload, 'application/json;charset=utf-8');
      showToast('Clipboard unavailable. Portal package JSON downloaded instead.', 'info');
    }
  };

  const parsePortalSubmission = (raw: any, index: number): EquityPortalSubmission => {
    const source = raw?.submission || raw;
    return {
      id: String(source.id || source.submissionId || `portal_submission_${Date.now()}_${index}`),
      token: String(source.token || source.packageToken || raw?.package?.token || ''),
      packageId: source.packageId || '',
      submittedAt: source.submittedAt || source.createdAt || new Date().toISOString(),
      investorName: String(source.investorName || source.name || 'Unnamed investor'),
      email: source.email || '',
      phone: source.phone || '',
      entityName: source.entityName || '',
      desiredAmount: toNumber(source.desiredAmount || source.amount),
      instrumentType: (source.instrumentType || 'undecided') as EquityReservationInstrument,
      signatureName: source.signatureName || source.typedSignature || source.investorName || '',
      ipHash: source.ipHash || '',
      userAgent: source.userAgent || '',
      consentElectronicRecords: !!source.consentElectronicRecords,
      acknowledgmentIndicationOnly: !!source.acknowledgmentIndicationOnly,
      acknowledgmentRisk: !!source.acknowledgmentRisk,
      status: (source.status || 'new') as EquityPortalSubmissionStatus,
      notes: source.notes || 'Imported from Cloudflare Investor Portal.',
    };
  };

  const importPortalSubmissions = () => {
    const text = portalImportText.trim();
    if (!text) return showToast('Paste Cloudflare submission JSON first.', 'error');
    try {
      const parsed = JSON.parse(text);
      const list = Array.isArray(parsed) ? parsed : Array.isArray(parsed.submissions) ? parsed.submissions : [parsed];
      const incoming = list.map((item, index) => parsePortalSubmission(item, index));
      if (!incoming.length) return showToast('No portal submissions found in that JSON.', 'error');
      const now = new Date().toISOString();
      onChange(prev => {
        const normalized = normalizeCompanyEquityState(prev, defaultBusinessName);
        const existingIds = new Set((normalized.portalSubmissions || []).map(s => s.id));
        const existingTokens = new Set((normalized.portalSubmissions || []).map(s => `${s.token}::${s.submittedAt}`));
        const merged = incoming.filter(s => !existingIds.has(s.id) && !existingTokens.has(`${s.token}::${s.submittedAt}`));
        return {
          ...normalized,
          portalConfig: { ...(normalized.portalConfig || defaultPortalConfig()), lastSyncAt: now },
          portalSubmissions: [...merged, ...(normalized.portalSubmissions || [])],
        };
      });
      setPortalImportText('');
      showToast(`Imported ${incoming.length} portal submission${incoming.length === 1 ? '' : 's'}.`, 'success');
    } catch {
      showToast('That portal JSON could not be parsed.', 'error');
    }
  };

  const updatePortalSubmissionStatus = (id: string, status: EquityPortalSubmissionStatus) => {
    onChange(prev => {
      const normalized = normalizeCompanyEquityState(prev, defaultBusinessName);
      return {
        ...normalized,
        portalSubmissions: (normalized.portalSubmissions || []).map(s => s.id === id ? { ...s, status } : s),
      };
    });
    showToast(`Portal submission marked ${status}.`, 'success');
  };

  const convertPortalSubmissionToReservation = (submission: EquityPortalSubmission) => {
    const now = new Date().toISOString();
    onChange(prev => {
      const normalized = normalizeCompanyEquityState(prev, defaultBusinessName);
      const existing = normalized.reservations.some(r => r.packageToken && r.packageToken === submission.token && r.email === submission.email);
      const reservation: EquityInvestmentReservation = {
        id: newId('eq_reservation'),
        date: todayIso(),
        investorName: submission.investorName,
        email: submission.email || '',
        phone: submission.phone || '',
        entityName: submission.entityName || '',
        desiredAmount: submission.desiredAmount,
        instrumentType: submission.instrumentType,
        status: 'confirmed',
        followUpDate: '',
        signatureName: submission.signatureName || submission.investorName,
        source: 'cloudflare_portal',
        packageStatus: 'signed',
        packageToken: submission.token,
        packageTitle: 'Private Investment Reservation',
        packagePreparedFor: submission.investorName,
        packageSignedAt: submission.submittedAt,
        consentElectronicRecords: submission.consentElectronicRecords,
        acknowledgmentIndicationOnly: submission.acknowledgmentIndicationOnly,
        acknowledgmentRisk: submission.acknowledgmentRisk,
        notes: `Converted from Cloudflare portal submission. ${submission.notes || ''}`,
        createdAt: now,
        updatedAt: now,
      };
      return {
        ...normalized,
        reservations: existing ? normalized.reservations : [reservation, ...normalized.reservations],
        portalSubmissions: (normalized.portalSubmissions || []).map(s => s.id === submission.id ? { ...s, status: 'converted' } : s),
      };
    });
    showToast('Portal submission converted into a confirmed reservation.', 'success');
  };
  ...
  const packageLinkFor = (record: Partial<EquityInvestmentReservation>) => portalConfig.portalBaseUrl ? buildPackageLink(record.packageToken || 'preview-token', portalConfig.portalBaseUrl) : (record.packageLinkPlaceholder || buildPackageLink(record.packageToken || 'preview-token'));
  ...
  const buildPackageRecord = (draft: Partial<EquityInvestmentReservation>): Omit<EquityInvestmentReservation, 'id' | 'createdAt' | 'updatedAt'> => {
    ...
    return {
      ...
      packageLinkPlaceholder: draft.packageLinkPlaceholder || buildPackageLink(token, portalConfig.portalBaseUrl),
  ...
  const secondaryNavItems: Array<{ id: EquitySection; label: string }> = [
    { id: 'portal', label: 'Cloudflare Portal' },
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 pb-24">
        ...
        <section className={`${cardClass} p-5 space-y-5`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
            {[
              ...
              ['4', 'Draft investor package', 'Create a private DocuSign-style package, then publish it to the Cloudflare investor portal.'],
              ['5', 'Import portal submissions', 'Import signed Cloudflare portal submissions, then convert accepted submissions into reservations or issued-share records.'],
            ].map(([num, title, body]) => (
        ...
        <section className={`${cardClass} p-5 space-y-6`}>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
              ...
              <p className={subTextClass}>Draft a DocuSign-style private reservation page for family/friends investors. This creates the owner-side package, Cloudflare portal payload, real public-link format, invitation text, and downloadable HTML preview.</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                <button onClick={() => copyPortalPackageJson(draftPreviewRecord())} className="rounded-xl border border-slate-300 dark:border-slate-700 font-extrabold px-4 py-3 flex items-center justify-center gap-2"><Copy size={16}/> Copy Portal JSON</button>
                <button onClick={() => downloadPortalPackageJson(draftPreviewRecord())} className="rounded-xl border border-slate-300 dark:border-slate-700 font-extrabold px-4 py-3 flex items-center justify-center gap-2"><Download size={16}/> Download Portal JSON</button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
                ...
                <div className="mt-1 break-all text-sm font-bold text-slate-700 dark:text-slate-200">{packageLinkFor(packageDraft)}</div>
                <p className="mt-2 text-xs font-semibold text-slate-500 dark:text-slate-400">Use this link after the Cloudflare investor portal is deployed and the package JSON is loaded through the Worker/D1 backend.</p>
              </div>
              ...
              <div key={r.id} className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => downloadPortalPackageJson(r)} className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 text-xs font-black flex items-center gap-1"><Download size={14}/> Portal JSON</button>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
      ...
      {activeSection === 'portal' && (
        <section className={`${cardClass} p-5 space-y-6`}>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div>
              <h2 className="text-xl font-black text-slate-950 dark:!text-white">Cloudflare Investor Portal</h2>
              <p className={subTextClass}>Connect the owner-side MONIEZI equity records to a public Cloudflare page where invited investors can review a private package and submit a signed indication of interest.</p>
            </div>
            <span className={statusPillClass(portalConfig.enabled ? 'green' : 'amber')}>{portalConfig.enabled ? 'portal configured' : 'setup needed'}</span>
          </div>

          <div className="rounded-2xl border border-blue-200 dark:border-blue-900/50 bg-blue-50 dark:bg-blue-950/20 p-4 flex gap-3">
            <Link2 size={20} className="text-blue-700 dark:text-blue-300 mt-0.5 flex-shrink-0" />
            <div className="text-sm font-semibold text-blue-900 dark:text-blue-100">
              <b>Portal scope:</b> MONIEZI Raise now includes the portal configuration, public-link format, package JSON export, submission JSON import, and a deployable Cloudflare Worker/D1 portal scaffold in <code>cloudflare/investor-portal</code>. Investors still do not see the private cap table, stock ledger, stakeholders, or internal notes.
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="md:col-span-2 rounded-xl border border-slate-300 dark:border-slate-700 px-3 py-3 flex items-center gap-2 font-bold">
              <input type="checkbox" checked={!!portalConfig.enabled} onChange={e => updatePortalConfig({ enabled: e.target.checked })} /> Enable Cloudflare portal workflow
            </label>
            <Field label="Investor portal public URL"><input className={fieldClass} value={portalConfig.portalBaseUrl || ''} onChange={e => updatePortalConfig({ portalBaseUrl: e.target.value })} placeholder="https://invest.yourdomain.com" /></Field>
            <Field label="Worker API URL"><input className={fieldClass} value={portalConfig.apiBaseUrl || ''} onChange={e => updatePortalConfig({ apiBaseUrl: e.target.value })} placeholder="https://moniezi-investor-portal.your-subdomain.workers.dev" /></Field>
            <Field label="Company slug"><input className={fieldClass} value={portalConfig.companySlug || ''} onChange={e => updatePortalConfig({ companySlug: e.target.value })} placeholder="emyrgen-group" /></Field>
            <Field label="Issuer/contact name"><input className={fieldClass} value={portalConfig.issuerName || ''} onChange={e => updatePortalConfig({ issuerName: e.target.value })} placeholder="Eli Green" /></Field>
            <Field label="Admin email"><input className={fieldClass} value={portalConfig.adminEmail || ''} onChange={e => updatePortalConfig({ adminEmail: e.target.value })} placeholder="issuer@example.com" /></Field>
            <Field label="Last sync"><input className={fieldClass} readOnly value={portalConfig.lastSyncAt || 'Not synced/imported yet'} /></Field>
            <div className="md:col-span-2"><Field label="Portal notes"><textarea className={fieldClass} rows={3} value={portalConfig.notes || ''} onChange={e => updatePortalConfig({ notes: e.target.value })} placeholder="Internal notes about Cloudflare deployment, D1 database, worker secret, custom domain, etc." /></Field></div>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
              <div className="text-lg font-black text-slate-950 dark:!text-white">1. Create package</div>
              <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">Draft the investor package in Investor Packages, then copy or download the Portal JSON.</p>
            </div>
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
              <div className="text-lg font-black text-slate-950 dark:!text-white">2. Load Cloudflare</div>
              <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">Use the Worker admin endpoint to create the package in D1. Send the generated <b>/p/token</b> link by email or text.</p>
            </div>
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4">
              <div className="text-lg font-black text-slate-950 dark:!text-white">3. Import submissions</div>
              <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">Export signed submissions from the Worker and paste the JSON below. Convert accepted submissions into MONIEZI reservations.</p>
            </div>
          </div>

          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
            <div>
              <h3 className="font-black text-slate-950 dark:!text-white">Import signed portal submissions</h3>
              <p className={subTextClass}>Paste the JSON response from the Cloudflare Worker <code>/api/admin/submissions</code> endpoint. MONIEZI stores it locally and lets you convert it into a confirmed reservation.</p>
            </div>
            <textarea className={fieldClass} rows={7} value={portalImportText} onChange={e => setPortalImportText(e.target.value)} placeholder='{"submissions":[{"token":"abc123","investorName":"John Smith","email":"john@example.com","desiredAmount":25000,"instrumentType":"common_stock","signatureName":"John Smith","consentElectronicRecords":true,"acknowledgmentIndicationOnly":true,"acknowledgmentRisk":true}]}' />
            <button onClick={importPortalSubmissions} className="w-full rounded-xl bg-blue-600 hover:bg-blue-700 !text-white font-extrabold px-4 py-3 flex items-center justify-center gap-2"><Download size={16}/> Import Portal Submissions</button>
          </div>

          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                <h3 className="font-black text-slate-950 dark:!text-white">Portal Submissions</h3>
                <p className={subTextClass}>Signed investor indications imported from the Cloudflare portal. These are not issued shares until you accept them and record a share issuance.</p>
              </div>
              <span className={statusPillClass('slate')}>{portalSubmissions.length} submission{portalSubmissions.length === 1 ? '' : 's'}</span>
            </div>
            {portalSubmissions.length === 0 ? (
              <div className="rounded-xl bg-slate-50 dark:bg-slate-900 p-4 text-sm font-semibold text-slate-500 dark:text-slate-400">No portal submissions imported yet.</div>
            ) : portalSubmissions.map(submission => (
              <div key={submission.id} className="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                  <div>
                    <div className="font-black text-slate-950 dark:!text-white">{submission.investorName} • {money(submission.desiredAmount, currencySymbol)}</div>
                    <div className="mt-1 flex flex-wrap items-center gap-2"><span className={statusPillClass(submission.status === 'converted' ? 'green' : submission.status === 'rejected' ? 'red' : 'blue')}>{submission.status}</span><span className={statusPillClass('slate')}>{formatInstrument(submission.instrumentType)}</span>{submission.email && <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{submission.email}</span>}</div>
                    <div className="mt-2 break-all text-xs font-semibold text-slate-500 dark:text-slate-400">Token: {submission.token || '—'} • Submitted: {submission.submittedAt}</div>
                    {submission.notes && <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{submission.notes}</p>}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => convertPortalSubmissionToReservation(submission)} className="px-3 py-2 rounded-xl bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300 text-xs font-black">Convert to Reservation</button>
                    <button onClick={() => updatePortalSubmissionStatus(submission.id, 'reviewed')} className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 text-xs font-black">Mark reviewed</button>
                    <button onClick={() => updatePortalSubmissionStatus(submission.id, 'rejected')} className="px-3 py-2 rounded-xl bg-red-50 text-red-700 dark:bg-red-950/30 dark:text-red-300 text-xs font-black">Reject</button>
                  </div>
                </div>
              </div>
            ))}
