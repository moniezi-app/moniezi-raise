...
export type EquityPortalSubmissionStatus = 'new' | 'imported' | 'reviewed' | 'converted' | 'rejected';
...
export interface EquityInvestmentReservation {
  ...
  source?: 'manual' | 'investor_form' | 'cloudflare_portal';
...
export interface EquityPortalConfig {
  enabled?: boolean;
...
export interface EquityPortalSubmission {
  id: string;
  token: string;
  packageId?: string;
  submittedAt: string;
  ...
  acknowledgmentIndicationOnly?: boolean;
  acknowledgmentRisk?: boolean;
  status: EquityPortalSubmissionStatus;
  notes?: string;
}
...
export interface CompanyEquityState {
  ...
  portalConfig?: EquityPortalConfig;
  portalSubmissions?: EquityPortalSubmission[];
}
